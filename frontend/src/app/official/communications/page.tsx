  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col antialiased">
      ...
      <PageHeader
        ...
        operationStatus="OBSERVE → ASSESS → PLAN → EXECUTE → MONITOR → REPLAN"
      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 max-w-7xl mx-auto w-full">
        ...
        <Card className="p-4 border-blue-200 bg-blue-50/70 text-blue-950 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Radio Nets Active</span>
              <Radio className="w-4 h-4 text-blue-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">4 Nets</p>
            <span className="text-[11px] text-slate-600 font-medium">CH-01, CH-02, CH-04, CH-08</span>
          </Card>

          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Registered Directory</span>
              <Users className="w-4 h-4 text-emerald-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900 mt-2">{mockContactsDirectory.length}</p>
            <span className="text-[11px] text-slate-600 font-medium">Verified field contacts</span>
          </Card>

          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Broadcast Messages</span>
              <Megaphone className="w-4 h-4 text-purple-600" />
            </div>
            <p className="text-2xl font-bold text-purple-700 mt-2">{broadcastFeed.length}</p>
            <span className="text-[11px] text-purple-700 font-medium">Dispatched directives</span>
          </Card>

          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Comms Desk</span>
              <ShieldCheck className="w-4 h-4 text-emerald-600" />
            </div>
            <p className="text-2xl font-bold text-emerald-700 mt-2">Online</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <form onSubmit={handleSendBroadcast} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full text-xs p-2 border border-slate-300 rounded-md bg-slate-50 font-sans focus:ring-2 focus:ring-slate-900/20"
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 text-xs border border-slate-300 rounded-md font-sans focus:ring-2 focus:ring-slate-900/20 resize-none"
                    required
                  />
                </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            {/* Filter & Search Bar */}
            <Card className="p-4 border-slate-200 bg-white shadow-xs space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredContacts.map((contact) => (
                <Card
                  key={contact.id}
                  className="p-4 border-slate-200 bg-white hover:border-slate-300 transition-all shadow-xs space-y-3"
                >
